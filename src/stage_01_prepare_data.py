...
def main(config_path, params_path):
    ...
    encode="utf8"
    with open(source_data_path, encoding=encode) as fd_in: #actual input data being read
        with open(train_data_path, "w", encoding=encode) as fd_out_train: #writing train data
            with open(test_data_path, "w", encoding=encode) as fd_out_test: #writing test data
                process_posts(fd_in=fd_in, fd_out_train=fd_out_train, fd_out_test=fd_out_test, target_tag=tag, split=split)
